...
class Database
{
     ...
     * @var \PocketDB\Client|null
     */
    public ?\PocketDB\Client $client = null;
    ...
    public function close(): void
    {
        ...
        foreach (array_keys($this->document_criterias) as $id) {
            if (isset(self::$criteria_registry[$id])) unset(self::$criteria_registry[$id]);
        }
    ...
    {
        if (!isset(self::$criteria_registry[$id])) return false;
    ...
    public static function match($criteria, $document)
    {
        ...
        foreach ($criteria as $key => $value) {
            ...
            switch ($key) {
                case '$and':
                    foreach ($value as $v) {
                        if (!self::match($v, $document)) return false;
                    }
                    break;
                ...
                default:
                    ...
                    if (\is_array($value)) {
                        if (!self::check($d, $value)) return false;
                    } else {
